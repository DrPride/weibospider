...
class Weibo(object):
    def __init__(self, config):
        ...
        self.got_comment_num = 0 # 存储爬到的微博评论数字 每次都要清空 
        self.weibo = []  # 存储爬取到的所有微博信息
        self.weibo_id_list = []  # 存储爬取到的所有微博id
        self.comments = [] # 存储爬到的微博评论数字 每次都要清空 
    ...
    def get_weibo_comment(self, weibo_id):
        try:
            ...
            req = requests.get(url.format(0), cookies=self.cookie)
            comment_num=1
            comment={}
            comments = []
            if req.status_code == 200:
                ...
                for page in tqdm(range(1, max_page+ 1)):
                    req = requests.get(url.format(page), cookies=self.cookie)
                    if req.status_code==200:
                        print('读取%s页的评论：'%page)
                        ...
                        for j in range(0,len(comment_page)):
                            ...
                            print('读取%s条评论'%comment_num)
                            keys = ['微博id','评论id','用户名','发布时间','评论内容','点赞数']
                            values = [weibo_id, comment_id, user_name, created_at, text, likenum]
                            comment = dict(zip(keys, values))
                            comments.append(comment)
                            comment_num += 1

                        self.got_comment_num = comment_num
                        self.comments = comments
                        
                        if page%10 == 0:
                            self.write_comment_csv(wrote_num)
                            wrote_num = self.got_comment_num

                        if page - page1 == random_pages and page < max_page:
                            sleep(random.randint(10, 20))
                            page1 = page
                            random_pages = random.randint(1, 5)
                self.write_comment_csv(wrote_num)
                print(u'%d条评论写入csv文件完毕,保存路径:'%wrote_num)
                print(self.get_filepath('comment.csv'))
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                self.comments = self.get_weibo_comment(weibo['id'])
                ## 
            else:
                weibo = None
            return weibo
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if type == 'img' or type == 'video':
                return file_dir
            if type == 'comment.csv':
                file_path = file_dir + os.sep + self.user_config['user_id']+ '_'+ type
                return file_path
    ...
    def write_comment_csv(self, wrote_num):
        try:
            result_headers = [
                '微博id',
                '评论id',
            ...
            result_data = [comment.values() for comment in self.comments[wrote_num:]]
            print("wrote_num: " ,wrote_num)
            # print('comments:           ', result_data)
            if True:  # python3.x
                with open(self.get_filepath('comment.csv'),
                          'a',
                          encoding='utf-8-sig',
                          newline='') as f:
                    writer = csv.writer(f)
    ...
    def get_weibo_info(self):
        ...
        try:
            ...
            self.write_data(wrote_num)  # 将剩余不足10页的微博写入文件
            if not self.filter:
